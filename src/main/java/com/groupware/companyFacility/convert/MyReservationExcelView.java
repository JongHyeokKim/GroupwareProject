...
public class MyReservationExcelView extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		response.setHeader("Content-Disposition", "attachment;filename=\"myReservationList.xls\";");
	      
	      HSSFSheet sheet = createFirstSheet(workbook); //페이지 정보(sheet) 설정
	      
	      createColumnLabel(sheet); //excel 문서의 컬럼명 세팅
		
	      List<ReservationVO> pageRanks = (List<ReservationVO>) model.get("myReservation"); //controller에서 setAtt한 거 가져옴.
	      int rowNum = 1;
		...
		private HSSFSheet createFirstSheet(HSSFWorkbook workbook) {
			...
			workbook.setSheetName(0, "예약리스트"); //(sheet 순번, sheet 제목)
