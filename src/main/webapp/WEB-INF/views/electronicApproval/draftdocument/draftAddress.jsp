...
<BODY onresize="parent.resizeTo(870,510)" onload="parent.resizeTo(870,510)">
	...
	<script type="text/javascript">
		...
		selectedTree = $.fn.zTree.init($("#selectedTree"), selectTreeSetting, selectedNode);
		$(function() {
				...
				success : function(keyData) {
					...
					$.ajax({
						...
						error : function(error) {
							swal("통신 에러 발생","부서 내 사원 로딩 에러 발생 " + error.message,"error");
						}
					});

				},
				error : function() {
					swal("통신 에러 발생","부서 로딩 에러 발생","error");
				}
			});
		...
		function addSelectMember(){
			...
			var selectCount = selectedTree.getNodes().length;
			if(memId==""){
				swal("사원 선택 에러","사원이 선택되지 않았습니다.","info");	
			}else if(selectCount<4 && selectedTree.getNodeByParam("id",memId,null)==null){
				selectedNode[selectCount] = {
						id : memId,
						name : $("#mem_nm").text(),
						position : $("#pos_nm").text(),
						isParent : false,
					};
				selectCount++;
				selectedTree = $.fn.zTree.init($("#selectedTree"), selectTreeSetting, selectedNode);
			}else if(selectCount==4){
				swal("최대 결재자 초과","결재자는 최대 4명까지만 선택 가능합니다.","info");
			}else{
				swal("중복 추가","이미 추가되있는 사원입니다.","info");
			}
		}
		...
		function finishSetting(){
			window.opener.sig_mem = selectedTree.getNodes();
			$(opener.location).attr("href", "javascript:finishSigMemSetting();");
			window.close();
		}
	</script>
	<style>
...
#address-down {
	...
	<div style="width: auto; float: left; margin-left: 10px;">
		...
		<div style="text-align: center; margin-top: 10px;">
			<button type="button" class="btn btn-default"
				onclick="openAllNodes()">전체열기</button>
			<button type="button" class="btn btn-default"
	...
	<div style="float: left; width: 40%; margin: 10px;">
		<table id="address" border="1">
			<tr>
				<td class="info-up-width info-image-height" rowspan="4"><img id="mem_img" src="treeview/images/defaultUser.png" width="118px" height="156px"
					style="margin: 5px;" /></td>
	...
	<div style="float: left; width: auto; margin-top:150px; text-align: center; margin-right: 10px;">
		<button type="button" class="btn btn-default" style="font-size: 5pt; padding: 5px"
				onclick="addSelectMember();" >>></button><br>
			<button type="button" class="btn btn-default" style="font-size: 5pt; padding: 5px"
				onclick="removeSelectMember();"><<</button>
	</div>
	<div style="float: left; width: auto;">
		<ul id="selectedTree" class="ztree"></ul>
		<div style="text-align: center; margin-top: 10px;">
			<button type="button" class="btn btn-default"  onclick="finishSetting();">확인</button>
			<button type="button" class="btn btn-default"
