...
<BODY onresize="parent.resizeTo(870,510)" onload="parent.resizeTo(870,510)">
	...
	<script type="text/javascript">
				...
				success : function(keyData) {
					...
					$.ajax({
						...
						error : function(error) {
							swal("사원 로딩 에러!", "부서 내 사원을 불러오는 중 에러가 발생했습니다.", "error");
						}
					});

				},
				error : function() {
					swal("부서 로딩 에러!", "부서를 불러오는 중 에러가 발생했습니다.", "error");
				}
			});
		...
		function addSelectMember(){
			...
			if(memId==""){
				swal("선택 에러!", "사원이 선택되지 않았습니다.", "warning");
				...
				swal("중복 에러!", "이미 추가되있는 사원입니다.", "error");
			}
		}
		...
		function finishSetting(){
			...
			$(opener.location).attr("href", "javascript:finishAddressSetting();");
			window.close();
		}
	</script>
	<style>
...
#address-down {
	...
	<div style="float: left; width: auto; margin-top:150px; text-align: center; margin-right: 10px;">
		<button type="button" class="btn btn-danger" style="font-size: 5pt; padding: 5px"
	...
	<div style="float: left; width: auto;">
		...
		<div style="text-align: center; margin-top: 10px;">
			<button type="button" class="btn btn-danger"  onclick="finishSetting();">확인</button>
			<button type="button" class="btn btn-danger"
